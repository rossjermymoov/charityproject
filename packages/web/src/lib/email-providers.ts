...
export async function testEmailProvider(
  ...
  try {
    const success = await sendViaProvider(provider, {
      ...
      subject: "Parity CRM — Email Provider Test",
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
          <h2 style="color: #111827;">Email Test Successful</h2>
          <p style="color: #374151;">
            This confirms your <strong>${provider.provider}</strong> email provider is working correctly in Parity CRM.
          </p>
  ...
  return sendViaSendGrid(
    {
      ...
      fromName: process.env.EMAIL_FROM_NAME || "Parity CRM",
    },
    options
  );
}
